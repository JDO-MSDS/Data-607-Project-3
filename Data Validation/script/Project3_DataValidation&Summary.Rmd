---
title: "Project 3: Data Science Skills Data Validation"
...
R was attempted to be pulled from Google Trends, however the team could not find an appropriate term to utilize, as searching for "R" returned various results unrelated to the R scripting language.

**Key Items We'll Answer & Summarize:**

- Any errors or missing values in the data?
- Explore graphs generated previously and summarize findings.
...
```{r}
raw_data <- read_csv("https://raw.githubusercontent.com/JDO-MSDS/Data-607-Project-3/refs/heads/main/Data%20Cleaning/data/data_science_skills_gt.csv", show_col_types = FALSE)

head(raw_data)

long_data <- read_csv("https://raw.githubusercontent.com/JDO-MSDS/Data-607-Project-3/refs/heads/main/Data%20Visualizations/trends_long.csv", show_col_types = FALSE)

head(long_data)
```

Split raw data by ",". Need to make all items of class character first.

```{r}
raw_data <- raw_data %>% mutate_all(as.character)

raw_data <- raw_data %>% separate(col = `Category: Programming`, into = c("Week","Python","SQL","Tableau"), sep = ",")

head(raw_data)

#Take out first row - old headers
raw_data<- raw_data[-1,]
```

Check for NAs. Apply function checks for NA values row-wise via second argument being 1.

```{r}
NA_set <- apply(raw_data,1,function(x) all(!is.na(x)))

#Any NA?
ifelse(NA_set, "", "THERE IS AN NA VALUE IN THIS ROW")
```

Verified that all rows have values.

## Relative to Absolute

I noted that the data pulled from Google Trends was pulled all together, which provides relative search amounts. Python appears to be searched much more often than SQL or Tableau. I exported data per search term and will explore below

Load each data set.

```{r}
tableau_data <- read_csv("https://raw.githubusercontent.com/JDO-MSDS/Data-607-Project-3/refs/heads/main/Data%20Validation/Data/Tableau.csv", show_col_types = FALSE)

python_data <- read_csv("https://raw.githubusercontent.com/JDO-MSDS/Data-607-Project-3/refs/heads/main/Data%20Validation/Data/python.csv", show_col_types = FALSE)

sql_data <- read_csv("https://raw.githubusercontent.com/JDO-MSDS/Data-607-Project-3/refs/heads/main/Data%20Validation/Data/sql.csv", show_col_types = FALSE)
```

Split, join, and make long

```{r}
tableau_data <- tableau_data %>% separate(col = `Category: All categories`, into = c("Week","Tableau"), sep = ",")
tableau_data<- tableau_data[-1,]

python_data <- python_data %>% separate(col = `Category: All categories`, into = c("Week","Python"), sep = ",")
python_data<- python_data[-1,]

sql_data <- sql_data %>% separate(col = `Category: All categories`, into = c("Week","SQL"), sep = ",")
sql_data<- sql_data[-1,]

comp_data <- left_join(python_data, sql_data)
comp_data <- left_join(comp_data,tableau_data)

head(comp_data)
```

Let's graph the trend lines and summarize absolute data and compare to the relative findings below. Need to make long first

```{r, fig.align="left"}
#make long & fix types
comp_long <- comp_data %>% pivot_longer(
  cols = c(Python, SQL, Tableau),
  names_to = "Skill_Name",
  values_to = "Interest"
)
comp_long <- comp_long %>% group_by(Skill_Name) %>% arrange(Skill_Name)

#make Week column to date and Interest numeric
comp_long$Week <- ymd(comp_long$Week)
comp_long$Interest <- as.numeric(comp_long$Interest)

#Plot trends
ggplot(comp_long, aes(x = Week, y = Interest, color = Skill_Name)) +
  geom_line(linewidth=1) +
  labs(
    title = "Google Search Interest: Data Science Skills Over Time",
    subtitle = paste("Weekly trends from", min(comp_long$Week), "to", max(comp_long$Week)),
    x = "Date",
...
ggplot(comp_long, aes(x = Week, y = Interest, color = Skill_Name)) +
  ...
  labs(
    ...
    subtitle = paste("Lines show smoothed trends with 95% confidence intervals"),
    x = "Date",
    y = "Search Interest Score (0-100)",
    color = "Skill",
    caption = "Data source: Google Trends"
...
comp_long %>%
  ...
  summarise(
    ...
    standard_dev = sd(Interest)
  ) %>%
  knitr::kable(caption = "Summary Statistics by Skill")
```

Averages and minimum interest are relatively close together. I don't remember what kind of statistical test to do but the averages are within much less than one standard deviation of each other so we can see the data is similar for each absolute data set.

# Summary

As a group we successfully obtained data from Google Trends, transformed the data into a long workable format, and applied several methods of analysis to determine what are the most valued data science skills, with the population of Python, SQL, and Tableau. As we can see below, Python is the strongest search term with SQL and Tableau at much less volume over time. When looking at the relative data, Python searches are declining after ~2022, while SQL and Tableau searches are relatively flatlined. However, the absolute trend graphs show that all three search terms tended to follow the same trend, especially starting around 2022 the trend lines stay close together. Of the data tested, we can conclude Python is the most valued data science skill.
...
# Previous analysis of relative data from Candace
##### Converting date column to date type to prep for data visulaizations and predictive analysis

```{r}
# Load the CSV
trends_viz <- read_csv("https://raw.githubusercontent.com/JDO-MSDS/Data-607-Project-3/refs/heads/main/Data%20Visualizations/trends_long.csv", show_col_types = FALSE)
...
```{r trends_timeline, fig.align="left"}
...
ggplot(growth_stats, aes(x = reorder(skill_name, percent_change), y = percent_change, fill = skill_name)) +
  ...
  theme(
    ...
    plot.margin = margin(1, 1, 1, 1)  # Add right margin
  )
```
